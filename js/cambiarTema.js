...
$(document).ready(function () {
    ...
    // Esta funcion se encarga de ver cual es el filtro seleccionado y poner las imagenes que correspondan.
      function filtrarContenido(filtro) {
        ...
        for (let i = 0; i < contenedores.length; i++) { // por cada contenedor
          ...
          if (filtro === "shakira-y-pique") { // Si el filtro es mas-relevantes, sacamos la data del array "orden 1"
            img.setAttribute("src", ordenTema_1_likes[i].img); // Le ponemos la imagen que corresponda quedando el elemento <img src="placaX.jpeg"/>
            contenedores_likes[i].innerText = ordenTema_1_likes[i].likes; // Le ponemos al contenedor de likes el número que corresponda.
            console.log(contenedores_likes[i].innerText);
          } else if (filtro === "concierto-tini") { // Si el filtro es mas-nuevos lo mismo, pero lo sacamos del array "orden 2"
            img.setAttribute("src", ordenTema_2_likes[i].img); 
            contenedores_likes[i].innerText = ordenTema_2_likes[i].likes;
          }
      ...
      function rellenarVerMas() {
        ...
        for (let i = 0; i < contenedores.length; i++) {
          ...
          img.setAttribute("src", cargar_mas_likes_tini[i].img);
          contenedores_likes[i].innerText = cargar_mas_likes_tini[i].likes;
      ...
      filtrarContenido("shakira-y-pique");
    ...
    /*----------------PLACAS DE TINI---------------------*/ 
    var ordenTema_2_likes = [
      { likes: 70, img: "img/ptini1.png" },
